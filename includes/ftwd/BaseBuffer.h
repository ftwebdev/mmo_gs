...
namespace tpl {
	...
	public:
		...
		virtual operator bool () const {
			return (_state && _state[BaseDynBuffData] != NULL);
		}
			// compare by addres
		...
		virtual T operator[] (const size_t n) const {
			size_t i = (n * sizeof(T));
			if (!isEmpty() && n < _state[BaseDynBuffLen]) {
				return ((T*)_state[BaseDynBuffData])[i];
			}
			throw(BaseDynamicBufferOutOfRange);
		}
		...
		/*
			...
			before including this file and then call the test by calling 'BaseDynamicBuffer<char>::test();'
			in your testing functions
			
			TODO: Yuo can make your own test functions, this is just a example
		*/
